...
- 多用户权限隔离还不完整；
- Milvus Lite 不适合长期高并发多人写入；
- 上传解析没有后台任务队列；
- 没有独立业务数据库保存用户、文档、权限、问答历史；
- 日志、监控、备份、错误追踪还比较基础；
- 如果多人长期使用，需要更严格的数据安全和访问控制。
...
1. 修复所有源码中文乱码，统一 UTF-8。
2. 扩大 `eval_questions.jsonl`，整理 50-100 个真实实验室问题。
3. 根据评测结果继续调整 chunk size、overlap、top_k 和 rerank threshold。
4. 增加页码、章节标题、表格行号等 metadata。
5. 上传和 embedding 改成后台任务，避免大文件上传时接口阻塞。
6. 增加用户登录、用户组、文档权限。
7. 将 Milvus Lite 升级为 Milvus Standalone、Qdrant 或 PostgreSQL + pgvector。
8. 增加问答历史、日志、错误追踪和备份机制。
9. 对答案来源做更细粒度可视化，例如显示命中的 chunk、高亮关键词、展示 rerank 分数。
10. 增加管理后台，用于查看文档、删除文档、重建索引、查看评测结果。

---

## 14. 一句话总结

这个项目已经完成了一个比较完整的实验室 RAG 原型系统：

```text
文件解析 + Chunk 切分 + Embedding + Milvus 向量库
+ BM25 混合检索 + Rerank 精排 + 低置信度拒答
+ 本地问答 + 联网问答 + 混合问答 + Cursor / Codex 辅助迭代生成的 Vue 前端工作台 + 基础评测
```

它现在适合学习、展示和实验室内部小规模试用。  
如果后续要给更多人长期稳定使用，下一步重点不是继续堆 Prompt，而是补齐权限、任务队列、独立数据库、服务化向量库和评测闭环。
